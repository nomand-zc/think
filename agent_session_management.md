...

{
  ...
  "message_ids": ["msg_id1","msg_id2"]  // 关联DeepAgents session的对话列表中的消息id
}
```
...
- 实时更新：新增块时，同步更新对应session的块索引库，并在session末尾追加轻量通知（如“新增块b_004，关键词：写入文件,report.txt”），让模型感知最新块的存在。

## 渐进式披露与自主检索模块（新方案新增）

核心目标：实现「按需供给上下文」，既保证session轻量化，又避免上下文缺失。
...
3. 工具在当前session的块索引库中检索，返回指定块的原始M条对话消息

4. Agent将召回的完整块对话消息拼接至当前上下文，生成回复；检索结果用完即弃，不常驻session，避免Token反弹。

5. 当模型生成返回内容后，追加到session中的完整历史对话列表中，并自动判断是否需要出发新的分块。

### 3. 元信息传递方式（适配不同块数量场景）

- 块数量＜10个：直接注入系统提示词，以结构化列表呈现所有块元信息，模型快速感知；
```plaintext
你是一个具备对话历史检索能力的智能体，需要根据用户问题，自主决策是否检索历史对话块。

【历史对话块概览】：（共3个块，块内是完整的语义闭环，检索时请使用块关键词或block_id）
1.  块ID：b_20250101_001 | 类型：tool_call | 关键词：读取文件,data.txt,2025年数据,store://xxx-001 | 核心语义：读取data.txt筛选2025年数据，结果存储至store://xxx-001
2.  块ID：b_20250101_002 | 类型：qa | 关键词：LLM微调,学习率,0.01 | 核心语义：讨论LLM微调学习率设为0.01，解决过拟合问题
3.  块ID：b_20250101_003 | 类型：chat | 关键词：部署,显存,GPU | 核心语义：讨论模型部署需要的GPU显存配置

【检索规则】
1.  当你的回答需要依赖上述块的信息时，调用search_list工具，传入检索关键词或block_id；
2.  检索工具会返回对应块的完整消息内容，你可以基于完整内容生成回答；
3.  不需要历史信息时，直接回答，无需调用工具。
...
```plaintext
你是一个具备对话历史检索能力的智能体。当前子任务下存在历史对话块，你可以：
1.  调用list_blocks工具，获取所有历史块的关键词和核心语义；
2.  根据list_blocks的结果，调用search_list工具检索具体的块（传入关键词或block_id）；
3.  不需要历史信息时，直接回答。
```
...
2. 可以通过块内容的多级索引来支持无限膨胀的上下文内容。理论上只要能实现动态多级索引即可。

### 5. 挑战点
1. 上下文中只保留1-2块最新内容 + 3-5条最新消息的情况（即使有历史块信息的情况下），是否会影响效果。理论上通过块消息检索能让模型可以获取任意想了解的历史消息
2. 在问题1的基础上，要求块信息质量高、分块质量也要高
