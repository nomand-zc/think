...
- **高频工具预加载**：基于工具元数据库的调用频次统计，筛选高频工具（如read/write/ls/grep/glob/todo等），与三大原子工具一同在Agent初始化时预加载至LLM。预加载列表支持动态调整，定期（如每日）更新调用频次，新增高频工具、移除低频工具。预加载仅注入工具核心信息（名称、简要描述、关键参数），减少Token占用。
...
- 缓存分片管理：将缓存分为“预加载工具缓存”“高频临时缓存”“低频临时缓存”，按优先级分配缓存资源，高频工具缓存时长 longer than 低频工具。
...
|预加载工具列表不合理|预加载工具占用过多Token，或高频工具未预加载|定期统计工具调用频次，自动调整预加载列表；提供手动配置入口，支持用户自定义预加载工具|
# 六、系统适配与扩展性
